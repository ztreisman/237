...
Each vertex v carries a CrystallineNeighborhood object that knows:
  ...
  - Scores against multiple lattices (Z[φ]³, FCC, E₈ shadow, ...)
...
Z[φ]³ fingerprint extracted directly from 237Lattice.vZome:
  - Every degree-7 vertex has spoke signature AABCCCC
    A: |e|≈1.000 (2 neighbors), type sq=(18,21) in Z[φ]
    B: |e|≈1.004 (1 neighbor),  type sq=(20,20) = pure φ power
...
# ---------------------------------------------------------------------------
# Z[φ]³ lattice constants
# ---------------------------------------------------------------------------

PHI = (1 + 5**0.5) / 2

# Squared lengths in Z[φ]: (a,b) means a + b*φ
...
def align_to_lattice(
    ...
    (i.e., what the neighbors *should* be at, in R³).
    """
    L = LATTICE_DB[lattice_key]
    offsets = neighbor_positions - center_pos
...
MEANINGFUL_GRAPH_DISTANCES = {
    # Klein quartic relations: word lengths in π₁ of the {3,7}/PSL(2,7) quotient
...
def snap_candidates(
    ...
    Vertices u (u≠v) such that:
      1. |pos[v] - pos[u]| < spatial_radius  (close in R³)
...
if __name__ == '__main__':
    ...
    print(f"Z[φ] spoke canonical: {np.round(ZPHI_SPOKE_CANONICAL,4)}")
    print(f"Z[φ] pairwise canonical (first 7): {np.round(ZPHI_PAIRWISE_CANONICAL[:7],4)}")
