...
def procrustes_align(pred, target):
    ...
    R = Vt.T @ U.T
    # Ensure proper rotation (det = +1). Vt is an SVD output already used
    # to compute R above -- in-place `Vt[-1] *= -1` bumps its autograd
    # version and breaks backward through the first R. Build a fresh
    # tensor instead.
    if torch.linalg.det(R) < 0:
        sign_fix = torch.eye(Vt.shape[0], device=Vt.device, dtype=Vt.dtype)
        sign_fix[-1, -1] = -1
        Vt = sign_fix @ Vt
        R = Vt.T @ U.T
    return pred_c @ R.T, R
...
class EGNNLayer(nn.Module):
    def __init__(self, h_dim, noise_emb_dim=16):
        super().__init__()
        # Edge: (h_i, h_j, rel_dist, L_ij, L_ij², σ_emb) → message
        ...
        # EGNN-style: start the coordinate-update output near zero, see
        # train_udg_embedder.py for the full rationale (instability fixes
        # ported from there -- this script's T_refine recycling makes
        # unbounded per-layer steps even more dangerous, since x passes
        # through the 6-layer stack up to 8x at sigma_max).
    ...
    def forward(self, h, x, edge_src, edge_dst, L, sigma_emb):
        ...
        dist = torch.sqrt((dv*dv).sum(-1, keepdim=True) + 1e-12)
        dv_hat = dv / dist
        # relative distance error, bounded to (-1,1), zero at target --
        # see train_udg_embedder.py: raw d2 fed an exponential blow-up
        # across layers (and here, across T_refine recycling passes too).
        ...
        # bounded step (|w|<1, softsign not tanh -- see
        # train_udg_embedder.py for why tanh flatlines gradients) of at
        # most one target edge-length, along the current direction.
        w = F.softsign(self.coord_net(msg))
        step = w * dv_hat * L.unsqueeze(-1)
...
class DenoisingEmbedder(nn.Module):
    ...
    def forward(self, x_noisy, edge_src, edge_dst, L, sigma, T_refine=1):
        ...
        """
        V = x_noisy.shape[0]
        # degree = count as src + count as dst (single direction per edge)
        ...
        # mirror edges: EGNNLayer's coordinate update only scatters to
        # edge_src, so without this the larger-indexed endpoint of every
        # edge never gets a direct positional update (see
        # train_udg_embedder.py for the full diagnosis).
        src2 = torch.cat([edge_src, edge_dst])
        dst2 = torch.cat([edge_dst, edge_src])
        L2 = torch.cat([L, L])
        sigma_emb = self.sigma_embedding(sigma, src2.shape[0], x_noisy.device)
        x = x_noisy - x_noisy.mean(0)   # center
        for _ in range(T_refine):
            h = h0.clone()
            for layer in self.layers:
                h, x = layer(h, x, src2, dst2, L2, sigma_emb)
        return x
...
class UDGDataset(torch.utils.data.Dataset):
    def __init__(self, npz_files, val=False, val_frac=0.15, seed=0):
        # base_id = q{q}_n{dim}_r{ring}_t{trial} (first 4 tokens) -- 5
        # tokens would also include the chord-length tag (L1/L2) and
...
def train_stage(model, files, args, stage_name, prev_ckpt=None):
    ...
    if prev_ckpt and os.path.exists(prev_ckpt):
        # weights_only=False: trusted, locally-generated checkpoint; older
        # checkpoints (and the npz-derived np.float64 fields) need full unpickling
        ck = torch.load(prev_ckpt, map_location=device, weights_only=False)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # cast to native floats: np.float64 pickles as numpy.core.multiarray.scalar,
        # which torch.load(weights_only=True) (PyTorch >=2.6 default) refuses to unpickle
        vp = float(np.mean(v_pos)); ve = float(np.mean(v_edge)); vs = float(np.mean(v_succ))
...
def main():
    ...
    ap.add_argument('--lib',  nargs='+', default=['udg_library'])
    ...
    ap.add_argument('--start-ring', type=int, default=1,
                    help='resume a curriculum partway through, warm-starting '
                         'from <ckpt-dir>/ring<start_ring-1>_best.pt')
    args = ap.parse_args()
    ...
    for lib in args.lib:
        ...
        all_files += [f for f in files if not f.endswith('_base.npz')]
    ...
    if args.curriculum:
        ...
        prev_ckpt = None
        if args.start_ring > 1:
            prev_ckpt = os.path.join(args.ckpt_dir, f'ring{args.start_ring-1}_best.pt')
            if not os.path.exists(prev_ckpt):
                raise SystemExit(f"--start-ring {args.start_ring} requires {prev_ckpt}")
        orig_epochs = args.epochs
        args.epochs = args.epochs_per_stage
        for r in range(args.start_ring, max_ring+1):
