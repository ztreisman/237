...
## Proposition 4 (Conditional capacity under quantitative reach —
## MOTIVATIONAL ONLY, not part of the proof: the reach hypothesis is alien
## to the problem, the threshold lands at ~13 not 6, and the bound vanishes
## as λ → 0. Retained because exponential-demand-vs-polynomial-room is what
## motivated the capacity question. Proof-bearing version: link_lemma.md.)

Say a realization f of P_r has **reach λ** if any two faces that do not
share a vertex are at Euclidean distance ≥ λ. Let C₀ = 16: the faces
sharing a vertex with a fixed root face number exactly 16 at interior
vertices — 1 root + 3 edge-sharing + 3 vertices × 4 vertex-only — or by
inclusion–exclusion over the three vertices, 3·7 − 3·2 + 1 = 16 (each edge
lies in 2 faces; the triple intersection is the root itself). Boundary
vertices have fewer faces, so 16 is the worst case.
...
fail at finite radius.** Numerically (C₀ = 16):
...
vertex; hence the faces whose neighborhoods contain x form a pairwise
vertex-sharing family, contained in the cluster of any one member, so the
covering multiplicity is ≤ C₀ = 16. So Σ vol(U_i) ≤ C₀ · vol(B(0, r + λ/2)). Sum the
